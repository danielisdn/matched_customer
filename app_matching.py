...
if file_a and file_b:
    ...
    if st.button("Load Data", width="stretch"):
        ...
        st.toast("Dữ liệu đã được tải thành công!", icon="✅")
...
# ================================
    ...
    if st.button("START MATCHING", width="stretch", type="primary"):
        start = time.time()
        ...
        with st.status("Đang xử lý dữ liệu...", expanded=True) as status:
            ...
            args = [(chunk, b_clean, blocking_map, threshold) for chunk in chunks]
            ...
            # Fix Pickling Error bằng cách sử dụng ThreadPoolExecutor thay vì ProcessPoolExecutor
            with concurrent.futures.ThreadPoolExecutor(max_workers=cpu) as exe:
                ...
                for f in concurrent.futures.as_completed(futures):
                    ...
                    progress_bar.progress(done / len(futures))
